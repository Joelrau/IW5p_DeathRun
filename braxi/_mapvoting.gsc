...
startMapvote() 
{
	...
	if(mapList.size == 0)
	{
		mapList[0] = addMapItem(getDvar("mapname"), "deathrun");
	}
	level.maps = getRandomSelection(mapList, 5); //size can be 0!
	while(level.maps.size < 2)
	{
		level.maps[level.maps.size] = addMapItem(getDvar("mapname"), "deathrun");
	}
	level.mapvote = 1;
	level notify("mapvote");
	level.mapitems = 3;
	if(level.maps.size < level.mapitems)
	{
		level.mapitems = level.maps.size;
	}
...
getRandomSelection(mapList, no, illegal) { //returns random selection of # elements of the maplist
	...
	m_size = randomValues.size; //number of legal indices
	maps = []; //maps in selection
	
	if (m_size <= no) { //number of entries is less than mapvote elements so we return full array
	...
	for (i=0; i<no; i++) //we select the maps here
	{
		...
		for (ii = rI; ii < m_size - 1; ii++) //we remove index from legal indices
		{
