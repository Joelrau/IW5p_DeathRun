...
main()
{
	...
	//maps\mp\gametypes\_healthoverlay::init();//level.healthRegenDisabled = true;
...
buildItemInfo()
{
	...
	for( idx = 1; isdefined( tableLookup( tableName, 0, idx, 0 ) ) && tableLookup( tableName, 0, idx, 0 ) != ""; idx++ )
	{
		...
		level.itemInfo[id]["item"] = "iw5_" + (tableLookup( tableName, 0, idx, 4 ) + "_mp");
...
buildKnifeInfo()
{
	...
	for( idx = 1; isdefined( tableLookup( tableName, 0, idx, 0 ) ) && tableLookup( tableName, 0, idx, 0 ) != ""; idx++ )
	{
		...
		level.knifeInfo[id]["item"] = "iw5_" + (tableLookup( tableName, 0, idx, 4 ) + "_mp");
...
playerConnect() // Called when player is connecting to server
{
	...
	{
		if(stringContains(self getGuid(), "bot"))
		{
			self thread braxi\_teams::setTeam( "allies" );
			self spawnPlayer();
		}
...
PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime)
{
	...
	if ( level.dvar["damage_hitmarkers"] && isPlayer( eAttacker ) && eAttacker != self )
	{
...
fastestTime()
{
	...
	if( !isDefined(trig) || !trig.size || trig.size > 1 )
...
endTimer()
{
	if( isDefined( self.finishedMap ) || level.freeRun )
		return;
